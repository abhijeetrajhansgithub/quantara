...
def main():
    ...
    db = Database("my_database_v6", dimensions=1024, auto_persist=True, index_paths={"default": "my_database_v6.index"})
    ...
    query = "What are the soul of ancient civilizations?"
    embedded = ollama.embeddings(model=EMBEDDING_MODEL, prompt=query)
    results = db.search_doc(embedded["embedding"], top_k=3, return_text_outputs=True, metric="cosine", basic=False)
    print(results)
    print(db.stats())
